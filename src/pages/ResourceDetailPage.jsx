      ...
      <div className="mx-auto flex max-w-[1600px] flex-col gap-6 px-4 py-4 sm:px-6 sm:py-5 lg:px-10 xl:px-14">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_300px]">
          <div className="flex flex-col gap-6">
            {/* 4. Creator profile */}
            <CreatorProfile creator={resource.creator} />

            {/* 5. Tabs + 6-9. Tab content */}
            <div>
              <ResourceTabs
                activeTab={activeTab}
              ...
              {activeTab === "Description" && <DescriptionTab resource={resource} />}
              {activeTab === "Preview" && <PreviewTab resource={resource} />}
              {activeTab === "Reviews" && (
                <ReviewsTab resource={resource} reviews={reviews} distribution={ratingDistribution} />
              )}
              {activeTab === "Discussion" && <DiscussionTab discussions={discussions} />}
            </div>
          </div>

          {/* 3. Sticky purchase card (desktop only) */}
          <PurchaseCard resource={resource} onBuy={handleBuy} />
        </div>
