      ...
      <div className="min-h-screen bg-[#0b0a14] text-white flex flex-col font-sans">
        ...
        <header className="bg-[#121124] border-b border-gray-800 px-6 py-4 flex justify-between items-center shadow-md">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-extrabold text-sm sm:text-base text-gray-100">{selectedCallSession?.topic}</h1>
              <p className="text-[10px] text-gray-500">Duration: {selectedCallSession?.duration_minutes} Mins • Secure Embed</p>
            </div>
          </div>
          <Button variant="outline" size="sm" onClick={() => setCurrentPage('profile')} className="border-red-500 text-red-500 hover:bg-red-600 hover:text-white text-xs rounded-xl font-bold transition-all">
            Disconnect & Exit
          </Button>
        </header>
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <iframe 
            src={`https://meet.jit.si/${selectedCallSession?.jitsi_room_name}#userInfo.displayName="${profile?.name || 'Learner'}"`} 
